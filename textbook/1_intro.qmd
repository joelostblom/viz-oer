...
<i>
The rusty doors of your spaceship creak open. Through blurry eyes, you see metal spires curving up into the sky. You have landed in the colony of Viridia. Strange faces surround you, eyeing you with disbelief. The Viridians whisper "how could an Earthling get here"? But Earth is gone. Its demise was a warning for civilizations across the galaxy. And you? You are its last messenger. 

What happened in the last days of Earth? Why did it spiral out of control? In the next chapters of your life, you will help the Viridians to answer these questions, to visualize the history of your home planet. And maybe, just maybe, to avoid the same fate.
<i/>
...
Before clicking on the hint and solution below to read our explanation of the two approaches,
...
aids in recalling the information later].
 <!--TODO ref? -->
 <!--TODO should this be moved to an intro chapter? -->

:::: {.callout-default .ex-hint}

Does one of the approaches focus more on the data
and the other more on the plot-specific details?
...
<!--TODO explain jargon declarative/imperative? -->
...
Just like that addition of numbers
contain all counts of the individual numbers,
maybe the addition of two strings
would contain all the characters from both the strings?
...
## Learning to speak with a grammar of graphics {#sec-learning-to-speak}
...
What do these grammars look like?
In brief,
...
<!--TODO Should I maybe replace it with a programmatic version? It is a fun image, not sure if it looks to informal -->
...
For example,
we could *encode* the column `Miles_per_Gallon`
...
It can be helpful to start with a particular area of the chart.
For example,
look at the cars with the most powerful engines (highest horsepower),
what is their fuel efficiency?
Now do the same for the cars with the least powerful engines 
and compare the two.
...
Altair will automatically figure out an appropriate color scale to use.
The `Weight_in_lbs` column contains continuous numerical data
so a continuous gradient will be used for the color scale.
...
When using the `color` encoding channel,
ggplot will automatically figure out which color scale to use.
The `Weight_in_lbs` column contains continuous numerical data
so a continuous gradient will be used for the color scale.
The largest value in the `Weight_in_lbs` column
...
<!--TODO should we learn this here? depends on how many figs will need it before it is introduced in a chapter -->
:::
...
you can try to start with a specific area of the chart,
first focusing on finding the fuel efficiency
of the cars with the highest horsepower,
and then compare it to the cars with the lowest horsepower.
...
While we have already outline the advantages of the libraries we chose,
it can be helpful to be aware of which others are available 
since you are likely to encounter some of them in the wild.
In this diagram
you can see an overview of how which other libraries exist
...
graph TD
    ...
    JS[JavaScript]:::javascript
```
As a reward for checking out the deep dive,
enjoy this genAI composition
of the visualization landscapes in R and Python.
...
**Minister Reed** wants to ensure that you have understood your task...
...
url = 'data/aggregated__food_data.csv'
food_emission = pd.read_csv(url)
```

```{pyodide}
#| exercise: initial_chart_alt
...
#| exercise: initial_chart_alt
#| check: true
solution = alt.Chart(food_emission).mark_point().encode(
...
url <- 'data/aggregated__food_data.csv'
food_emission <- readr::read_csv(url, show_col_types=FALSE)
```

```{webr}
#| exercise: initial_chart_gg
# TODO change column names so that backticks are not required
ggplot(______) +
    aes(
        x=`______`,
        y=`______`,
...
# }
list(correct = TRUE, message = "This check is not working yet")
```
...
For a reminder of the syntax,
review section @sec-learning-to-speak.
...
With altair you can add a `tooltip` just as you have been using
`x`, `y`, and `color`.
Try it out to investigate which foods these are.

```{pyodide}
#| exercise: tooltip_chart_alt
alt.Chart(food_emission).______.______(
    ______
    ______
    tooltip=______,
)
