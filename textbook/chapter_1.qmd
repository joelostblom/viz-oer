...
This means that we are not going to have one function called `barplot`,
another called `scatterplot` and so on.
Instead we are going to add the three components together,
as we will see in the next se
This is in contrast Unlike traditional charting tools that offer pre-made chart types,
a grammar-based approach is more flexible. It allows the user to construct custom visualizations from fundamental building blocks.
