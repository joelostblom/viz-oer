...
#### Grammar for Altair and ggplot

Two of the most prominent declarative statistical visualization libraries are [Altair](https://altair-viz.github.io/) (Python and R) and [ggplot](https://ggplot2.tidyverse.org/). These offer a powerful and concise visualization grammar for quickly building a wide range of statistical graphics. 
...
In the previous image of the plotting landscape, you’ll notice various high-level tools. While these tools are more convenient than low-level ones, their syntax differs from Altair and ggplot. They typically use a top-down approach, focusing on the type of plot to build (e.g., `histogram(data, x, y, color)`) rather than building from the data (e.g., `canvas(data).mark_bar().encode(bin(x), y, color)`). Both approaches have their benefits, but this course emphasizes the grammar-based, bottom-up approach due to its flexibility and power.