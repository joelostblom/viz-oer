...
As we have seen in the exercises above, grouping can have major benefits. There are two main purposes where grouping is useful:
...
These purposes are often connected and can be explored simultaneously through a common grouping strategy. They supports us in making more accurate conclusions about our data. The example from the exercises on alcohol consumption and life expectancy fall under the second category: the data seemed to have a certain pattern when we looked at all the points together, but when grouping by another variable in the dataset, we discovered that this overall pattern could be due to confounding from another variable. 

Another name for this purpose is Simpson's Paradox, named not after Homer Simpson et al., but rather its inventor Edward H. Simpson, a British codebreaker.  Simpson reported a curious phenomenon where a trend present in several different groups of data disappears when they are combined. Below is a more general and simplified illustration of Simpson's Paradox than the one in our exercise. Notice how the overall dataset does not have a clear direction but when you color by group, both groups exhibit the same negative trend between X and Y.
...
In this case, grouping makes the insidious Simpson's Paradox visually unmistakable. This is a prime example of the utility of visualization in deepening our understanding of a data set. 

We can also use grouping to directly compare trends and values for distinct groups. Here, grouping by position on the y-axis reveals that life expectancies are higher in countries in the higher income groups. 
...
income_df <- read_csv('data/income_lifeexp_alcohol.csv', show_col_types = FALSE)
...
Do you think that traffic lights are well designed? At first glance, they break a major rule of accessibility, using the worst possible color combination for people with deuteranopia (red and green). Thankfully, in many countries, traffic lights are encoded using both position (vertically along the "y-axis"), color, and shape.

![](/img/traffic.png)

Grouping by three encoding channels reinforces the visual contrast and has probably prevented many accidents.
Even in traffic light signals for cars that often lack the shape encoding, the vertical positioning of the different signals is effective enough so it is not a major hazard.
...
The syntax for encoding a shape aesthetic, is the same as for color: 
...
It is rarely effective to encode a different data set variable
...
that it is notably more difficult
to tell apart the four different sizes,
than the four different colors in the chart above.
...
Notice how easy it is to compare the distributions between regions? This is because the separation between the groups is much clearer when using distinct positions than when using distinct colors, shapes, or sizes for points with overlapping positions.
...
Now, using the variables `region`, `income_group` and `life_expectancy_at_65`, create a plot that reflects these priorities, showing all countries grouped using position and color. 
