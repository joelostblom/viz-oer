...
Coloring reveals that both national alcoholism levels and life expectancy are correlated with income. <span style="color: #9CCCC8; font-weight: bold">High income</span> countries tend to have longer lifespans and a greater proportion of heavy alcohol drinkers. 
...
The human brain can detect and process color differences with very little cognitive effort. Color is a **pre-attentive feature** which means that the brain distinguishes between colors even without conscious attention. This ability is heightened when there is a high contrast between the colors, as is the case with complimentary colors like blue and orange.

This is great news for grouping. By encoding our categorical variable as color, we leverage the brain's ability to infer groups in our visualization. For scatterplots, this works particularly well when there fewer than eight categories. Conveniently, color can be applied without altering the layout or structure of the data visualization, making it an efficient way to compare the exact values of data points between groups.
...
The power of color fades without accessibility. It's best to avoid using overly similar colors as this blurs the difference between groups. Apply color when you have a small number of groups: adding too many different colors overwhelms the visualization and it harder to intepret. Finally,  be cognizant of visual impairments. Colorblindness affects approximately 8% of men and 0.5% of women globally. The most common variant is deuteranopia, which impairs the ability to distinguish between red and green hues. A word of caution: while Altair’s default color palette is designed to be color-blind friendly, ggplot’s is not.
...
Shape can be used strategically in combination with color to provide an additional visual cue.

A classic example of this is the humble traffic light. At first glance, traffic lights seem terribly designed: they use the worst possible hues for people with deuteranopia (red and green). Thankfully, in many countries, traffic lights are encoded using both color and shape. A third encoding channel - position - is then used to establish predictable y-axis positions for both 'Stop' and 'Go' signals. Grouping by three encoding channels reinforces the visual contrast and has prevented many accidents.
...
While this combined approach makes grouping clearer, beware of the pitfalls. Line icons limit the total area that can be colored, reducing the effectiveness of color, so is preferable to opt for solid shapes when combining these encoding channels. 

Size matters more than you'd think. Larger shapes are easier to distinguish. When plotting a dense scatterplot of microscopic marks, try restricting the number of unique shapes, or consider avoiding using shape altogether.


### Size

While size usually represents the magnitude of quantitative data, it can, under limited circumstances, be applied to grouping. Because size has an inherent direction, it works better for **ordinal** categories - those with a clear order. 
...
As you've probably noticed, it takes more cognitive effort to interpret size-based groupings compared to colors. Restricting size to four or fewer levels is a good rule of thumb, and for most visualizations grouping by size should be avoided.

### Position

Position is the most interpretable grouping strategy. It involves mapping a categorical variable directly onto the x or y axis, turning each group into a clear visual lane. By keeping the same scale for the other axis, it allows you to compare distributions across categories. It also emphasizes outliers within each category, making them easier to spot.

Notice how grouping by position on the y-axis makes it easy to compare the distributions of heavy alcohol consumption across world regions:
...
Grouping by position is not all bunnies and rainbows. It sacrifices an axis that could have been used for a continuous variable, losing information. Visually separating each group can also obscure overall trends in the dataset and requires significant space.

As you saw in the visualization above, plotting data points along a single axis leads to significant overlap. That's where **jittering** comes to the rescue. It introduces slight variation in position of data points within each lane, making them easier to distinguish. 
...
While jittering is an attractive strategy, it can cause confusion. Jittering can easily be mistaken for a scatterplot with variation in the underlying data.
