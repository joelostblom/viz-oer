...
The human brain can detect and process color differences with very little cognitive effort. Color is a **pre-attentive feature** which means that the brain distinguishes between colors even without conscious attention. This is heightened when there is a high contrast, such as with complimentary colors like blue and orange.

This is great news for grouping! By encoding our categorical variable as color, we can leverage the brain's ability to infer groups. For scatterplots, this works best with fewer than eight categories. Conveniently, color doesn't alter the layout or structure of the data visualization, so it's an efficient way to compare the exact values of data points between groups.
...
But beware... The power of color fades when used irresponsibly. Overly similar colors can blur the difference between groups. Color works best when you have a small number of groups, otherwise your rainbow mess becomes overwhelming to interpret. And that's not to mention accessibility. Colorblindness affects approximately 8% of men and 0.5% of women globally. The most common variant is deuteranopia, which impairs the ability to distinguish between red and green hues. While Altair’s default color palette is designed to be color-blind friendly, ggplot’s is not.
...
Now our visualizations are really taking shape... Shape can be used strategically in combination with color to provide an additional visual cue.

A classic example of this is the humble traffic light. At first glance, traffic lights seem poorly designed: they use the worst possible hues for people with deuteranopia (red and green). Thankfully, in many countries, traffic lights are encoded using both color and shape. A third encoding channel - position - is then used to establish predictable y-axis positions for both 'Stop' and 'Go' signals. Grouping by three encoding channels reinforces the visual contrast and has probably prevented many accidents.
...
While this combined approach makes grouping clearer, watch out for some pitfalls. Line icons limit the total area that can be colored. This reduces the effectiveness of color, so its preferable to use solid shapes.

Size matters more than you'd think. Smaller shapes are difficult to distinguish. When plotting a dense scatterplot of small marks, try restricting the number of unique shapes, or consider avoiding using shape altogether.


### Size

Size is often used to represent the magnitude of quantitative data. But it can under limited circumstances be used for grouping. Because size has an inherent direction, it works better for **ordinal** categories - those with a clear order. 

One example is World Bank income level, ranging from low to high. Let's shake up our life expectancy chart, encoding income level of countries in sub-Saharan Africa as size:
...
As you can see, size-based groupings are not very intuitive. Restricting size to four or fewer levels is a good rule of thumb, and for most visualizations grouping by size should be avoided.

### Position

Position is a powerful grouping strategy. To do it, we encode a categorical variable as the x or y axis, turning each group into a clear visual lane. This makes it easy to compare distributions across categories. It also emphasizes outliers within each category, making them easier to spot.

Notice how grouping on the y-axis allows us to compare the distributions of heavy alcohol consumption across world regions:
...
Position can be combined with color to reinforce grouping in a category, or to group by a second categorical variable. As position is more visually obvious than color, try to reserve it for the primary comparison you are trying to make. 

::: {.column-margin}

**Think before you group**

In general, before applying grouping, consider how critical each 
categorical comparison is to your analysis. The most important comparisons should be assigned to the most effective grouping channels, which in order are position, color, shape and size.
:::

Grouping by position has drawbacks. For one, it occupies an axis that could have represented a continuous variable. This reduces the information density of your visualization. Visually separating each group by position can also obscure overall trends in the dataset and requires significant space.

There is another potential problem. As you saw in the visualization above, plotting data points along a single axis can lead to substantial overlap. That's where **jittering** comes to the rescue. It introduces slight variation in position of data points within each lane, making them easier to tell apart. 

::: {.panel-tabset}

# Altair

In Altair, we append the `.transform_calculate()` function to our chart to create a calculated variable called `jitter`. We then use the `yOffset` encoding channel. By setting `yOffset='jitter:Q'` we shift the `yOffset` of each point by a random value. The `:Q` simply tells Altair that `jitter` is a continuous quantitative variable.
...
:::
While jittering is an attractive strategy, it can cause confusion. Jittered points can easily be mistaken for a scatterplot with true variation in the underlying data.
...
Faceting is a radical alternative to grouping. Rather than using encoding channels, it creates a separate chart for each group. You can think of it like an extreme version of grouping by position, with the added benefit of not sacrificing an axis for a categorical variable.

::: {.panel-tabset}

# Altair

In Altair, to facet by income group we simply append `.facet('income_group')` to our chart.
