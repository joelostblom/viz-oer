...
A surprising correlation emerged: 50 year olds can expect to live longer in countries with a greater percentage of heavy alcohol drinkers. Great news! The researchers now plan to introduce alcohol into Viridia.
...
death_rate_df = pd.read_csv("data/life_expectancy_at_50_alcohol.csv")

alt.Chart(death_rate_df).mark_point().encode(
    x='percentage_heavy_drinkers',
    y='life_expectancy_at_50',
    tooltip=['country', 'life_expectancy_at_50', 'percentage_heavy_drinkers']
)
```

Try hovering over the data points to see which mark corresponds to each country. Can you see any patterns?

Suggest one visual change that could provide a more nuanced picture of the relationship between alcohol consumption and life expectancy.
...
library(ggplot2)
simpsons_df <- read.csv("data/simpsons_paradox_data_l.csv")

ggplot(simpsons_df, aes(x = X, y = Y, color = Group)) +
  geom_point()
```
...
ggplot(simpsons_df, aes(x = X, y = Y, shape = Group, color=Group)) +
  geom_point()
```
...
While size is more commonly used to represent quantitative data, it can under limited circumstances be effective in distinguishing between categories. It works better for **ordinal** categories that have a clear order. 
...
ggplot(simpsons_df, aes(x = X, y = Y, size = Group)) +
  geom_point()
```
